...
class bookingController extends Controller
{
    ...
    {
        $select = "select *,EXTRACT(MONTH FROM dateDeparture) month,EXTRACT(day FROM dateDeparture) date,hour(dateDeparture) dateDeparture_hour,minute(dateDeparture) dateDeparture_minute,hour(dateArrival) dateArrival_hour,minute(dateArrival) dateArrival_minute,timestampdiff(day,dateDeparture,dateArrival) day,mod(timestampdiff(hour,dateDeparture,dateArrival),24) hour,mod(timestampdiff(minute,dateDeparture,dateArrival),60) minute from planes,flights,routes,categories,airline_companies
    ...
    public function store_booking(FillRequest $request)
    {
        $bill = bill::create([
        ...
        if($bill){
            bill_detail::create([
                'bill_id' => $bill->id,
                'chair' => $request->chair."-".$request->chair,
        ...
        return view('payment')->with('msg','sucess');
    }
}
