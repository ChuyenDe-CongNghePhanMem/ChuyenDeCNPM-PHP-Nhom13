...
<body>
    <div id="main">
        ...
        <form action="{{  route('store_booking')}}" method="POST">
            @csrf
            <input name="count_customer" value="{{ $request->count_customer }}" type="text">
            @foreach ($flights as $flight)
            <input name="chair" type="number" value="{{ $flight->chair }}">
            <input name="departure" value="{{ $flight->dateDeparture }}" type="text">
            @for ($i=1;$i<=$request->count_customer;$i++)
            <input name="gender{{ $i }}" id="gender{{ $i }}" type="text">
            @endfor
            <?php
            $total_money=(float)$flight->price*(int)$request->count_customer;
            ?>
            <input name="total_money" type="number" value="{{ $total_money }}">
        ...
        @endfor
        <button type="submit">tuyen</button>
        </form>
    </div>
    <div id="right">
        @foreach ($flights as $flight)
        <div style="width: 100%">
            ...
            <div id="image" style="background-image:url('http://localhost:8080/laivanvo/public/images/Boeing 707.jpg');width:100%">
            <div class="boder"></div>
            <div class="info">
                {{ $flight->date }}-{{ $flight->month }} 
            </div>
            <div class="info">
                {{ $request->count_customer }} hành khách
            </div>
            <div class="info">
                {{ $flight->name_category }}
            </div>
            <div class="boder"></div>
            </div>
            ...
            <div id="detail_img"><img style="height: 95%" src="http://localhost:8080/laivanvo/public/images/planes/flight.png"/></div>
                <div id="detail_content">
                    ...
                    <div class="detail_content_item">
                        <img src="http://localhost:8080/laivanvo/public/images/vietnam airline.png" style="max-width: 10%;max-height:100%"/>
                ...
                <div class="detailUp" onclick="detailUp1()" id="detailUp"><img style="max-height:100%" src="images/up.png"/></div>
        ...
        <div id="total_money">
            ...
            <div style="width:40%;relative;float:left">Người lớn</div>
            <div style="text-align:right;width:60%;position: relative;float:right">{{ $flight->price }}đ x {{ $request->count_customer }}</div>
            <div style="width:40%;relative;float:left">Tổng cộng:</div>
            <?php
            $total=(float)$flight->price*(int)$request->count_customer;
            ?>
            <div style="text-align:right;width:60%;position: relative;float:right">{{ $total }}</div>
        </div>
        @endforeach
    </div>
